...
print("="*70)
print("STEP 1: Creating backup of 2024 BDL game outlook files")
print("="*70)

# Create backup
backup_dir = f'data/2024_data/mlb_data/raw/bdl_data/game_outlook_backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
os.makedirs(backup_dir, exist_ok=True)

outlook_files = sorted(glob.glob('data/2024_data/mlb_data/raw/bdl_data/game_outlook/*.csv'))
...
boxscore_files = sorted(glob.glob('data/2024_data/mlb_data/raw/boxscores/*.csv'))
all_boxscores = []
...
for mlb_date, group_df in outlook_df.groupby('mlb_date'):
    ...
    if mlb_date in game_pk_order_by_date:
        ...
        if len(ordered_rows) > 0:
            ...
            # Write to file
            outlook_file = f'data/2024_data/mlb_data/raw/bdl_data/game_outlook/game_outlook_{mlb_date}.csv'
...
for boxscore_file in boxscore_files[:10]:
    ...
    outlook_file = f'data/2024_data/mlb_data/raw/bdl_data/game_outlook/game_outlook_{date_str}.csv'
...
# Final count
final_outlook_files = glob.glob('data/2024_data/mlb_data/raw/bdl_data/game_outlook/*.csv')
