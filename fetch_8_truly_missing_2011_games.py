#!/usr/bin/env python3
"""
Fetch the 8 truly missing games from 2011 BDL game outlook data.
"""
...
# Missing games to fetch (dates from MLB)
MISSING_DATES = [
    "2011-04-12",  # KC @ MIN
    "2011-04-21",  # OAK @ SEA
    "2011-04-26",  # STL @ HOU
...
# Expected matchups for verification
EXPECTED_MATCHUPS = {
    "2011-04-12": ("KC", "MIN"),
    "2011-04-21": ("OAK", "SEA"),
    "2011-04-26": ("STL", "HOU"),
...
def main():
    print("=" * 80)
    print("Fetching 8 Missing 2011 BDL Game Outlook Games")
    print("=" * 80)
    ...
    for date_str in sorted(set(MISSING_DATES)):
        expected_away, expected_home = EXPECTED_MATCHUPS[date_str]
        
        print(f"\n📅 Fetching games for {date_str} (looking for {expected_away} @ {expected_home})...")
        games = fetch_all_games_for_date(date_str)
        
        if games:
            print(f"  Found {len(games)} total games on this date")
            
            # Look for our specific matchup
            for game in games:
                home_abbr = normalize_team(game.get('home_team', {}).get('abbreviation', ''))
                away_abbr = normalize_team(game.get('away_team', {}).get('abbreviation', ''))
                
                if away_abbr == expected_away and home_abbr == expected_home:
                    print(f"  ✅ Found target game: {away_abbr} @ {home_abbr}")
                    all_fetched_games.append(game)
                    found_count += 1
                    break
            else:
                print(f"  ⚠️  Target game {expected_away} @ {expected_home} not found on this date")
        else:
    ...
    print(f"\n" + "=" * 80)
    print(f"Found {found_count} of 8 target games")
    print("=" * 80)
    ...
    # Save to CSV
    output_file = "fetched_8_missing_2011_games.csv"
    df.to_csv(output_file, index=False)
    
    print(f"\n✅ Saved {len(df)} games to: {output_file}")
    print("\nFetched games summary:")
    ...
    print("  2. Add them to the appropriate date files")
    print("=" * 80)
