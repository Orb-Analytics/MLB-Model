...
create_final_dataset = repo_root / "scripts" / "etl" / "builders" / "create_final_dataset.py"

if create_final_dataset.exists():
    subprocess.run([sys.executable, str(create_final_dataset)], check=True)
else:
    print(f"   Warning: {create_final_dataset} not found, skipping...")

# Note: clean_final_dataset.py no longer exists, skipping
