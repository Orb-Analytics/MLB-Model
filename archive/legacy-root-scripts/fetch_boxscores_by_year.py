...
def fetch_boxscores_for_year(year):
    """
    Fetch team boxscores for a specific MLB season year
    Matches the exact format of 2024 boxscores
    """
    print(f"\n{'='*60}")
    print(f"Fetching MLB Team Boxscores for {year} Season")
    print(f"{'='*60}\n")
    
    # Set output directory
    output_dir = f'/workspaces/MLB-Model/data/{year}_data/mlb_data/raw/boxscores'
    os.makedirs(output_dir, exist_ok=True)
    
    # Define date range (broad range to catch all games)
    # Most seasons: late March - late September
    # 2020 COVID season: July 23 - September 27
    start_date = datetime(year, 3, 1)
    end_date = datetime(year, 10, 15)
    
    # Special handling for COVID-shortened 2020 season
    if year == 2020:
    ...
    total_games = 0
    dates_processed = 0
    team_game_counts = defaultdict(int)  # Track game counts per team
            ...
            if 'dates' in schedule_data and len(schedule_data['dates']) > 0:
                for date in schedule_data['dates']:
                    for game in date.get('games', []):
                        # Only include regular season games (gameType='R')
                        if game.get('gameType') == 'R':
                            games.append(game)
            
            if games:
                print(f"📊 {date_str}: Processing {len(games)} games")
                dates_processed += 1
                ...
                for game in games:
                    ...
                    try:
                        ...
                        row = {
                            ...
                            'home_team_name': home_team_info.get('name', ''),
                            'away_team_name': away_team_info.get('name', ''),
                            'home_postseason': 0,  # Regular season only
                            'away_postseason': 0,
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python fetch_boxscores_by_year.py <year>")
        print("Example: python fetch_boxscores_by_year.py 2023")
        sys.exit(1)
